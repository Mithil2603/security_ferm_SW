...
async function test() {
  ...
  const sql = fs.readFileSync('src/database/migrations/018_financial_reporting.sql', 'utf8');
  const adapted = adaptMigrationSQL(sql);
  const stmts = adapted.replace(/--.*$/gm, '').split(';').map(s=>s.trim()).filter(s=>s.length>0);
  for(let s of stmts){
    try {
      await c.query(s);
