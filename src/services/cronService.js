  ...
  try {
    ...
    for (const rv of recurring) {
      ...
      if (lastCounter) {
        ...
        await query(
          "UPDATE voucher_counters SET last_number = $1 WHERE id = $2",
          [nextNum, lastCounter.id],
        );
      } else {
        await query(
          `
      ...
      await query(
        "UPDATE recurring_vouchers SET next_run_date = $1 WHERE id = $2",
        [nextRunDate, rv.recurring_id],
      );
    }

    await query("COMMIT");
    logger.info(
      `Successfully processed ${recurring.length} recurring vouchers.`,
    );
  } catch (err) {
